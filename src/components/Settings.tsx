...
export const Settings: React.FC<Props> = ({ onSaved }) => {
  const [provider, setProvider] = useState<'auto'|'free'|'mock'|'polygon'>('auto')
  const [polygonKey, setPolygonKey] = useState('')

  useEffect(()=>{
    try {
      const p = window.localStorage.getItem('bbs.provider') as 'auto'|'free'|'mock'|'polygon'|null
      setProvider(p || 'auto')
  ...
  return (
    <QPopover trigger={<button className="text-xs px-3 py-1.5 rounded-xl border border-slate-700/60 hover:bg-slate-800/60">Settings</button>}>
      <div className="space-y-3">
        ...
        <div className="text-xs opacity-80">Choose Auto, Free (Yahoo/Binance), Mock, or Polygon (requires key). Free uses public endpoints and needs no signup.</div>
